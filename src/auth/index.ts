...
export class ProfileStore {
  ...
  async create(input: { name: string; email: string; role?: string; title?: string; color?: string }): Promise<LawyerProfile> {
  ...
  async update(id: string, patch: Partial<Pick<LawyerProfile, "name" | "title" | "color" | "role">>): Promise<LawyerProfile> {
    const p = this.get(id);
