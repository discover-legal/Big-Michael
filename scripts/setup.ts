...
function askSecret(prompt: string, hasCur = false): Promise<string> {
  ...
  return new Promise(res => {
    ...
    const onData = (buf: Buffer) => {
      ...
      if (ch === "") { cleanupSecret(); process.stdout.write("\n"); process.exit(0); }
      if (ch === "\r" || ch === "\n") {
        cleanupSecret();
        process.stdout.write("\n");
        res(val);
...
async function multiSelect(title: string, rows: SelectRow[], preSelected: string[] = []): Promise<string[]> {
  ...
  const render = () => {
    ...
    // Viewport: clamp to terminal height with scroll window around cursor
    let lines: string[];
    if (allLines.length <= maxVisible) {
      lines = allLines;
    } else {
      // Header is first 3 lines; footer is last 2; content rows in between
      const headerLines = allLines.slice(0, 3);
      const footerLines = allLines.slice(-2);
      const contentLines = allLines.slice(3, -2);
      const viewContent = maxVisible - 5; // space for header + footer + scroll hint
      const half = Math.floor(viewContent / 2);
      let start = Math.max(0, cursor - half);
      let end = Math.min(contentLines.length, start + viewContent);
      if (end - start < viewContent) start = Math.max(0, end - viewContent);
      const scrollHint = fg.gray(`  ↕ ${cursor + 1}/${navigable.length}  (scroll with ↑↓)`);
      lines = [...headerLines, ...contentLines.slice(start, end), scrollHint, ...footerLines];
    }
  ...
  return new Promise(res => {
    ...
    // Buffer partial escape sequences (arrow keys can split across data events)
    let escBuf = "";

    const onData = (buf: Buffer) => {
      escBuf += buf.toString("utf8");
      // Wait for complete escape sequence
      if (escBuf === "\x1b" || escBuf === "\x1b[") return;

      const ch = escBuf;
      escBuf = "";

      if (ch === "") { cleanupPicker(); process.stdout.write("\n"); process.exit(0); }
...
// Strip control characters that would break .env parsing or enable key injection
function sanitizeEnvValue(s: string): string {
  return s.replace(/[\x00-\x1F\x7F]/g, "").trim();
}

function writeEnv(env: Record<string, string>, pythonBin: string) {
  const v = (k: string) => sanitizeEnvValue(env[k] ?? "");
  const lines = [
  ...
  // Atomic write + restrict to owner-only before rename so secrets are never world-readable
  fs.writeFileSync(tmpPath, lines.join("\n") + "\n", { mode: 0o600 });
  fs.renameSync(tmpPath, envPath);
  try { fs.chmodSync(envPath, 0o600); } catch { /* rename preserves mode; ignore if chmod unsupported */ }
}
...
`);
  ...
  // Pre-select any connector whose keys are already configured in env
  ...
  const preSelected = navigableIds.filter(id => {
    ...
    return conn?.keys.some(k => !!env[k.key]);
  });

  const chosen = await multiSelect("Choose connectors:", PICKER_ROWS, preSelected);

  // Clear keys for deselected connectors so stale values don't persist
  const chosenSet = new Set(chosen);
  for (const id of navigableIds) {
    if (!chosenSet.has(id)) {
      const conn = CONNECTORS[id];
      if (conn) for (const k of conn.keys) delete env[k.key];
    }
  }

  // Collect keys for each selected connector
  // DocuSeal key is only obtainable after Docker first-run — prompt for it after Docker step
  for (const id of chosen) {
    if (id === "docuseal") continue;
    const conn = CONNECTORS[id];
  ...
  // setup.sh already ran npm install — default to skip to avoid duplication
  const bootstrapped = !!process.env.BIG_MICHAEL_SETUP_BOOTSTRAPPED;
  const doInstall = await confirm("Run npm install?", !bootstrapped);
  let installed = bootstrapped; // treat as installed if bootstrapper already did it
  if (doInstall) {
  ...
  if (sys.docker) {
    ...
    if (doDocker) {
      ...
      try {
        ...
        dockerusealStarted = true;
        printNote("Visit http://localhost:3000 to complete first-run setup, then come back here.");
      } catch {
  ...
  // If DocuSeal was selected and Docker just started, prompt for the API key now
  if (chosenSet.has("docuseal") && dockerusealStarted) {
    console.log();
    printNote("Once you've completed DocuSeal first-run setup:");
    printNote("  Settings → API → copy the key");
    console.log();
    await collectKeys(CONNECTORS["docuseal"], env);
    // Re-write env with the freshly entered DocuSeal key
    writeEnv(env, pyR.bin);
    printOk(".env updated with DocuSeal key.");
  } else if (chosenSet.has("docuseal") && !dockerusealStarted) {
    printNote("DocuSeal selected but Docker not started — enter key later via:  npm run setup");
  }
  ...
  if (doSmoke) {
    const spin = new Spinner();
    spin.start("npm run smoke-test  (uses placeholder keys — checks wiring, not API validity)");
    try {
...
`);
    ...
    printWarn(`Dependencies not installed yet. Run ${fg.bold("npm install")} then ${fg.bold("npm run dev")} to start.`);
  }
